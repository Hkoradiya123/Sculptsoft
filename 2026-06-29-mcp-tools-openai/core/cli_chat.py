...
class CliChat(Chat):
    ...
    async def _process_command(self, query: str) -> bool | None:
        ...
        if command not in prompt_names:
            print(f"Error: Unknown command '/{command}'. Available commands: {', '.join('/' + name for name in prompt_names)}")
            return False

        if len(words) < 2:
            print(f"Error: Missing document ID for '/{command}'. Usage: /{command} <doc_id>")
            return False
    ...
    async def _process_query(self, query: str) -> bool:
        command_status = await self._process_command(query)
