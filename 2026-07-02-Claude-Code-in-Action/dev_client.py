...
async def run_client(ws_url: str):
    ...
    # --- REPLACE YOUR PREVIOUS STEP 1 CODE WITH THIS (Around Line 105) ---
    global wav_file_out
    recording_filename = f"recording_{int(time.time())}.wav"
    wav_file_out = wave.open(recording_filename, "wb")
    wav_file_out.setnchannels(CHANNELS)
    wav_file_out.setsampwidth(1)  # 1 byte for mulaw
    wav_file_out.setframerate(TWILIO_SAMPLE_RATE)
...
async def _send_mic_audio(ws, stream_sid: str, ui: TerminalUI):
    ...
    with stream:
        while True:
            ...
            # --- ADD THESE 3 LINES HERE (Around Line 169) ---
            with wave.open(recording_filename, "ab" if seq > 2 else "wb") as wav_file:
                wav_file.setnchannels(CHANNELS)
                wav_file.setsampwidth(1)
                wav_file.setframerate(TWILIO_SAMPLE_RATE)
                wav_file.writeframes(mulaw)
            # ------------------------------------------------
        ...
        async for message in ws:
            ...
            if event == "media":
                ...
                if payload:
                    ...
                    # --- ADD THESE 5 LINES HERE (Around Line 206) ---
                    with wave.open(recording_filename, "ab") as wav_file:
                        wav_file.setnchannels(CHANNELS)
                        wav_file.setsampwidth(1)
                        wav_file.setframerate(TWILIO_SAMPLE_RATE)
                        wav_file.writeframes(mulaw_bytes)
                    # ------------------------------------------------
